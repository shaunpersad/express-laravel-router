...
function createRouter(app, mapActionToHandler = (action) => action) {
    ...
    class Router {
        ...
        url(name, params = {}) {
            ...
            const { uri, constraints } = namedUrl;
            ...
            for (const character of uri) {
                ...
                switch(character) {
                    ...
                    case '}':
                        ...
                        if (constraints[currentParam] && !constraints[currentParam].test(p[currentParam])) {

                            throw new Error(`The value "${p[currentParam]}" for the param "${currentParam}" fails the "${constraints[currentParam]}" constraint.`)
                        }
                        parsingParam = false;
                        currentParam = '';
                        newUri+= encodeURIComponent(p[currentParam]);

                        delete p[currentParam];
                        break;
                    case '?':
                        optional = true;
                        break;
            ...
            const queryParams = Object.keys(p);
